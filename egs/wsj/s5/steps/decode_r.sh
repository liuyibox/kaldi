...
for f in $graphdir/HCLr.fst $graphdir/Gr.fst $data/feats.scp $model $extra_files; do
...
if [ $stage -le 1 ]; then
  $cmd $queue_opt JOB=1:$nj $dir/log/decode.JOB.log \
    nnet3-latgen-faster-r$thread_string $ivector_opts $frame_subsampling_opt \
     ...
     $graphdir/HCLr.fst $graphdir/Gr.fst "$feats" "$lat_wspecifier" || exit 1;
fi
